...
- **✍️ 생성** — 목록에서 여러 제품을 체크해 한 번에 생성 → 검토·복사·발행
- **📋 작업 현황** — 미작업/초안/발행을 한눈에 보고 다음 할 일 구분

### 기본 흐름
1. [🛰️ 수집] 탭 → 카테고리 번호(`cate_no`)와 수집 개수 입력 → [수집 시작]
   → `Product_eduino`에 제품 폴더가 자동 생성되고 상세이미지가 저장됩니다.
   (직접 폴더를 만들어 넣어도 됩니다.)
2. [✍️ 생성] 탭 → 왼쪽 목록(미작업만 기본)에서 **생성할 제품을 체크** →
   [선택한 N개 생성] (동시 병렬, 편당 과금)
3. 오른쪽 **검토 패널**에서 제품을 골라 제목·본문·메타·태그 원클릭 복사,
   관련 링크 채우고 네이버에 붙여넣어 `[이미지 N]` 자리에 사진 삽입 → 발행
4. [✅ 발행 완료로 표시] → [📋 작업 현황]에서 진행 상태 추적
